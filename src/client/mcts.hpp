...
namespace tree {
	...
	template<typename T>
	Node<T> * find_child(Node<T> *parent, T const& t) {
...
namespace mcts {
	...
	Node* select(Node *node, Game &game, int verbose=0) {
		...
		*/
		...
		assert(0);
		throw std::runtime_error("impossible to be here");
	}
	...
	void step(Node *root, Game &game, int verbose) {
		...
		while (node) {
			if (node->action)
				node->win += game.get_result(node->action->pid);
			node->total += 1.0f;
			node = node->parent;
		}
	...
	template <typename T>
	void apply(tree::Node<T> *&root, T const& action) {

		auto iter = tree::find_child(root, action);
		...
		if (iter) {
			...
			unique_ptr<T> tmp(action.copy());
			node = tree::insert_child(root, tmp);
		}
