...
namespace front {
	...
	namespace input{
		...
		SDLButtonMask mod_to_SDLButtonMask(Mod mod) {
			return SDLButtonMask(mod >> 25);
		};			

		SDLScancode key_to_SDLScancode() {
			assert(SDLK_SCANCODE_MASK & key);
			return ~SDLK_SCANCODE_MASK & key;
		}
		...
		struct Keyboard{
			...
			bool operator[](Key const key) const {			
				auto ind = key_to_SDL_Scancode(key);
				assert(ind < N);
				return state[ind];
			}
		...
		struct Mouse{
			...
			bool operator[](Key const key) const {			
				return button & SDL_BUTTON(key_to_SDL_BUTTON(key));
			}
			
			Mod get_mod() const {
				return SDL_BUTTON_MASK_to_mod(button);
			}
